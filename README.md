...
The dataset for this problem used with the `main.py` and `increased_accuracy.py` script is downloaded from here:
...
Please make sure to install all dependencies before running the frontend, using the following:

```
pip install -r requirements.txt
```
## Training Models with Enhanced Accuracy

To enhance the accuracy and cross-validation scores of the Decision Tree and Random Forest models while reducing overfitting, utilize the `increased_accuracy.py` script.

Run the script as follows:

python increased_accuracy.py

## Interactive Frontend

For running an interactive frontend or sharing it with others, please run `frontend.py`.
```
python frontend.py
```
